...
class database:
    ...
    def getDictFromQueryText(self, query = None, condition = None):
        ...
        cur = self.db.cursor(MySQLdb.cursors.DictCursor)
        if query == None:
            query =  "select distinct g.minute, p.name as player_name, f.datetime, f.stadium, f.city, f.group_name, f.teamHome, f.teamAway, p.number, p.image, p.position, p.birthdate from goals g join games_fifa_all f on g.matchId = f.match_fifa_id join players p on p.playerId = g.playerId "
